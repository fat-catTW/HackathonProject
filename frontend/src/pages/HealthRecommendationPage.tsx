    ...
    <>
      <main className="mx-auto min-h-dvh max-w-md bg-canvas px-5 pb-32 pt-8">
        <header className="flex items-center gap-3 pb-4">
          ...
          <h1 className="text-xl font-black text-slate-900">健康商品推薦</h1>
        </header>
        ...
        <section className="rounded-[24px] border border-white bg-white p-5 shadow-sm">
          ...
          >
            ...
            <button
              ...
              className="min-h-[44px] rounded-2xl bg-brand px-5 py-3.5 font-bold text-white disabled:opacity-40"
            >
              查詢
            </button>
          </form>
        ...
        {loading && (
          <p className="mt-6 text-center text-sm text-gray-400">正在為你尋找推薦商品…</p>
        )}
        ...
        {!loading && recommendations && recommendations.length > 0 && (
          <section className="mt-6 flex flex-col gap-3">
            {fallbackUsed && (
              <p className="rounded-2xl bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-700">
                這次是用關鍵字比對挑選的，僅供參考。
              </p>
            )}
            {recommendations.map((rec) => (
              <button
                ...
                className="min-h-[44px] rounded-2xl border-2 border-gray-200 bg-white p-4 text-left transition hover:border-brand disabled:opacity-60"
              >
                <p className="text-base font-black text-slate-900">{rec.name}</p>
                <p className="mt-1 text-sm leading-6 text-slate-500">{rec.reason}</p>
        ...
        {selectedProduct && (
          <section className="mt-6 rounded-[24px] border-2 border-brand bg-white p-5 shadow-sm">
