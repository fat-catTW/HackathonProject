...
- **只有這三項能沿用**：抽欄位時餵給模型的記憶就只有地址／電話／慣用時段
  （`agent._reusable_preference_context`），不含「上次摘要」——那行帶著上一張單的每一格，
  模型看到就會把上次的問題描述、叫修工項照抄進來，變成使用者這次沒說過的話被送去給廠商。
  另外描述類欄位（問題描述、備註）只收使用者這次講出口的內容，模型自己生的一律丟掉，
  寧可留空再問一次；「幫我填」本身是指令不是描述，不會被寫進問題描述。
