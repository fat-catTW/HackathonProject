"""Static + DynamoDB-backed catalog for the health product recommendation feature.

Product data is kept as a static fallback list here (same convention as
restaurant_catalog.py / delivery_catalog.py), while also being seedable into
DynamoDB (PK=PRODUCT#<id>, SK=METADATA, entity_type=PRODUCT) via
scripts/seed_health_products_catalog.py, mirroring the SERVICE#/METADATA
catalog pattern used by services/catalog.py + lambda_tools so this backend
and the deployed Lambda/MCP tools can eventually read from one shared table.

Data ported from the standalone 711-health-mcp prototype's
src/data/products.json (38 simulated 7-11 products).
"""
...
def _product_from_item(item: dict) -> dict:
    """DynamoDB numbers come back as Decimal; normalize to plain int/float."""
    return {
        "id": item["id"],
        "name": item["name"],
...
def list_products() -> list[dict]:
    """DynamoDB-first (entity_type=PRODUCT), falling back to the static list
    when the table has not been seeded yet (mirrors shared_lambda catalog's
    SERVICE_CATALOG_FALLBACK behavior)."""
    try:
        items = STORE.scan_by_entity_type("PRODUCT")
    except Exception:
        items = []
    if items:
...
def get_product(product_id: str) -> dict | None:
    try:
        item = STORE.get_item(f"PRODUCT#{product_id}", "METADATA")
    except Exception:
        item = None
    if item:
