"""Shared page knowledge helpers for Lambda MCP tool handlers."""
...
PAGE_CATALOG_PATH = ROOT / "page_knowledge" / "pages.json"

CURRENT_PAGE_HINTS = ("這頁", "這一頁", "目前頁面", "當前頁面", "現在這頁", "current page")
NAVIGATION_HINTS = (
    "怎麼去",
    "怎麼到",
    "去哪裡",
...
def _text_fragments(page: dict) -> list[str]:
    fragments = [
        page.get("page_id", ""),
        page.get("route", ""),
        page.get("title", ""),
...
def _is_application_query(text: str) -> bool:
    return any(hint in text for hint in APPLICATION_HINTS)
...
def search_pages(query: str, current_page_id: str | None = None, *, limit: int = 5) -> list[dict]:
    ...
    navigation_query = _is_navigation_query(normalized_query)
    application_query = _is_application_query(normalized_query)
    current_page_query = _is_current_page_query(normalized_query)
    ...
    for page in load_pages():
        ...
        for fragment in _text_fragments(page):
